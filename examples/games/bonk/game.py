    ...
    def __init__(self, position, color, size, name, screen_height, keys=None):
        ...
        self.domain = Box(
            (self.sprite.position.x,
             self.sprite.position.y, self.sprite.position.z),
            ...
            self.domain.update_position(
                (self.sprite.position.x, self.sprite.position.y,
                    self.sprite.position.z), (self.sprite.position.x + self.sprite.size.x,
                                              ...
                                              self.sprite.size.y, self.sprite.position.z
                                              + self.sprite.size.z))
        if message == "left":
            self.sprite.position.x -= 10
            self.domain.update_position(
...
class Bounce(object):
    ...
    def __call__(self, td, objects):
        ...
        if self.domain != None:
            for p1 in objects:
                check_score = self.domain.check_score(
                    p1.sprite.last_position, p1.sprite.position, objects)
                if check_score != "score":
                    collide_point, normal = self.domain.intersect(
                        p1.sprite.last_position, p1.sprite.position)
                    if collide_point is not None:
                        reflect_vec = (p1.sprite.position - collide_point).reflect(
                            normal) * bounce
                        p1.sprite.velocity = p1.sprite.velocity.reflect(
                            normal) * bounce
        ...
        for d in domains:
            domain = d
            for p1 in objects:
                collide_point, normal = domain.intersect(
                    p1.sprite.last_position, p1.sprite.position)
                if collide_point is not None:
                    reflect_vec = (p1.sprite.position - collide_point).reflect(
                        normal) * bounce
...
if __name__ == '__main__':
    ...
    win.clear()
    paddle = Paddle((10, 200, 0), (1.0, 0.0, 0.0, 1.0), (
        10.0, 100.0, 0.0), "paddle", win.height, keys=("up", "down"))
    ball = ball(position=(210, 100, 0), velocity=(200, 200, 0),
                color=(0.0, 0.0, 1.0, 1.0), size=(10.0, 0.0, 0.0))
    paddle2 = Paddle((620, 200, 0), (1.0, 0.0, 0.0, 1.0), (
        10.0, 100.0, 0.0), "paddle2", win.height, keys=("w", "s"))
